...
function hasConsecutiveSequenceCyclic(arr, length, startAt = '', max = 13) {
  ...
  //console.log(sorted)
  //console.log(Array.from(dupicateVals).sort((a, b) => value_of(a) - value_of(b)));
  //console.log("need length: "+length)
  //console.log("Start at: "+startAt)
  if(startAt != ''){
    if(sorted[0] != startAt) return false;
  }
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    for (let j = 1; j < length && count < max; j++) {
      const nextVal = (current + j - 1) % max + 1;
      if (!uniqueVals.has(nextVal)){
        //console.log("dont got: "+nextVal)
         return false;
      }
      count++;
    }
  ...
  if (dupicateVals.length >= length - countPass) {
    return hasConsecutiveSequenceCyclic(dupicateVals, length - countPass, 1, max);
  }
...
function analyze_spread() {
  ...
  if (spread.length >= 4) {
    ...
    for (var q = 4; q < 52 && q < spread.length + 1; q++) {
    ...
    if (spread[spread.length - 1].charAt(1) == spread[(spread.length - 2)].charAt(1) && spread[spread.length - 1].charAt(1) == spread[(spread.length - 3)].charAt(1) &&
      spread[spread.length - 3].charAt(1) == spread[(spread.length - 4)].charAt(1)) {//Four of a Kind
    ...
    else if (spread[spread.length - 1].charAt(1) == spread[(spread.length - 3)].charAt(1) && spread[spread.length - 2].charAt(1) == spread[(spread.length - 4)].charAt(1)) {//One-Two One-Two
    ...
    else if (spread[spread.length - 1].charAt(1) == spread[(spread.length - 4)].charAt(1)) {//Split Pair
      //Mirror Mirror
    ...
    else if (spread[spread.length - 4].charAt(1) == spread[(spread.length - 2)].charAt(1) && spread[spread.length - 4].charAt(0) == spread[(spread.length - 3)].charAt(0) && spread[spread.length - 2].charAt(0) == spread[(spread.length - 1)].charAt(0)) {//Double Date
    ...
    else if (spread[spread.length - 3].charAt(1) == spread[(spread.length - 1)].charAt(1) && spread[spread.length - 4].charAt(0) == spread[(spread.length - 3)].charAt(0) && spread[spread.length - 2].charAt(0) == spread[(spread.length - 1)].charAt(0)) {//Double Date
    ...
    if (spread[spread.length - 1].charAt(0) == spread[(spread.length - 4)].charAt(0)) { // Split Suits
    ...
    if (getUniqueSuits(spread.slice(-4)) == 4) {//Rainbow
  ...
  if (spread.length >= 3 && spread[spread.length - 1].charAt(1) == spread[(spread.length - 3)].charAt(1) && spread[spread.length - 1].charAt(1) == spread[(spread.length - 2)].charAt(1)) {//Three of a Kind
  ...
  if (spread.length >= 2) {
    if (spread[spread.length - 1].charAt(1) == spread[(spread.length - 2)].charAt(1)) {//Pair
  ...
  if (CARDCOUNT == 52 && spread.length > 2 && card_combo_chain.length == 0 && spread[spread.length - 1].charAt(1) == spread[0].charAt(1)) { /////alert("Bookend Finish... You Win!!");
  ...
  if ((CARDCOUNT == 52 && spread.length <= 4 && card_combo_chain.length == 0) || bookendwin) {
    ...
    if (bookendwin) { 
      if(spread.length == 3){
        ObjById('winning').innerHTML = "Winner!!!"; ObjById("displaytext").firstChild.nodeValue = "&iexcl; Bookend Three Ball !";
        winflag = 1;
        score *= 1.3; 
      }else{
      ObjById('winning').innerHTML = "Push"; PUSHEND = 1; ObjById("displaytext").firstChild.nodeValue = "A Bookend Finish!"; 
    }
    }
      ...
      ObjById("displaytext").firstChild.nodeValue = "The Han Solo..."; score *= 1.75;
    }
    else if (spread.length == 2) {
      winflag = 1;
      ObjById('winning').innerHTML = "Winner!!!";
      if (spread[spread.length - 1].charAt(0) == spread[0].charAt(0)) { 
        ObjById("displaytext").firstChild.nodeValue = "The Two Amigos..."; score *= 1.5; }
      else {
        ObjById("displaytext").firstChild.nodeValue = "The Two Strangers";
...
function card_click(card) {
  ...
  if (in_spread(card) && litcombo >= 0) {
    ...
    if (score_combo[x] > 0) { multi++; multiCancel=1; }
    litcombo = (-1)
    refresh_spread()
...
function pos_of(card) {
  ...
  for (i = 0; i < spread.length; i++) { if (card == spread[i]) return i }
  return (-1)
}

function in_spread(card) {

  for (i = 0; i < spread.length; i++) { if (card == spread[i]) return 1 }
  return 0
}
function in_pile(card) {

  for (i = 0; i < pile.length; i++) { if (card == pile[i]) return 1 }
  return 0
}
...
function flip_card(card) {
  ...
  if (crd[0].style.display == 'none') {
    ...
    ObjById(card).style.filter = `hue-rotate(0deg)`;
...
function swap_face_to_back(face) {
  ...
  const hue = getComputedStyle(document.documentElement).getPropertyValue('--card-back-hue').trim();
  face.style.filter = `hue-rotate(${hue})`;
}
